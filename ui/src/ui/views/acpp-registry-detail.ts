...
function eventDotColor(event: string): string {
  ...
  if (event === "task.failed" || event === "agent.error") {
...
function eventLabel(event: string, payload?: Record<string, unknown>): string {
  const raw = payload?.description ?? payload?.error ?? payload?.outcome ?? payload?.reason ?? "";
  ...
  return html`
    <section class="card">
      ...
      ${
        events.length === 0
          ? html`
              <div class="muted" style="padding: 16px 0">No activity events yet.</div>
            `
          : html`
              <div
                style="
