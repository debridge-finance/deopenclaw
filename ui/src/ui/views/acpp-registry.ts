...
function statusSummary(agents: AcppAgentSummary[]): string {
    ...
    if (counts["UNRESPONSIVE"]) parts.push(`${counts["UNRESPONSIVE"]} unresponsive`);
    if (counts["OFFLINE"]) parts.push(`${counts["OFFLINE"]} offline`);
    if (counts["DEREGISTERED"]) parts.push(`${counts["DEREGISTERED"]} deregistered`);
        ...
        ${!selectedAgent
            ? html`
                <div class="card">
                  ...
                  <div class="card-sub">
                    Pick a registered agent to inspect details, health, and activity.
                  </div>
                </div>
              `
